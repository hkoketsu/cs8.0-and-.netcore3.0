    ...
    static void AggregateProducts()
    {
        using (var db = new Northwind())
        {
            WriteLine(
                "{0,-25} {1,10}", 
                arg0: "Products count: ",
                arg1: db.Products.ToArray().Count());

            WriteLine(
                "{0,-25} {1,10:$#,##0.00}",
                arg0: "Highest product price:",
                arg1: db.Products.ToArray().Max(p => p.UnitPrice));
            
            WriteLine(
                "{0,-25} {1,10:N0}",
                arg0: "Sum of units in stock",
                arg1: db.Products.ToArray().Sum(p => p.UnitsInStock)
            ); 

            WriteLine(
                "{0,-25} {1,10:N0}",
                arg0: "Sum of units on order:",
                arg1: db.Products.ToArray().Sum(p => p.UnitsOnOrder)
            );

            WriteLine(
                "{0,-25} {1,10:$#,##0.00}",
                arg0: "Average unit price:",
                arg1: db.Products.ToArray().Average(p => p.UnitPrice)    
            );
